...
Currency formatting is determined by users locale currency. The currency formatting is stored in the `Shops.currencies` object, and the configuration data is available in the client as `EFrameworkCore.Locale`
...
Typical usage is `price = accounting.formatMoney price, EFrameworkCore.Locale.currency`
